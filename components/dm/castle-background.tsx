...
export function CastleBackground({ videoUrl, environmentName }: CastleBackgroundProps) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      {/* Fallback: Static Background (shown when no video or video loading) */}
      {(!videoUrl || !videoLoaded || videoError) && (
        <>
          {/* Base dark gradient */}
      ...
      {/* Environment label (when video is active) */}
      {videoUrl && videoLoaded && !videoError && environmentName && (
        <div className="absolute bottom-4 left-4 px-3 py-1 bg-black/60 backdrop-blur rounded border border-green-900/30">
          <span className="text-[10px] text-green-400 uppercase tracking-wider">{environmentName}</span>
        </div>
      )}

      {/* Vignette (always shown) */}
      <div 
