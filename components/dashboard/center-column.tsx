...
function FeaturedSpeaker({ speaker, line, voiceId, caption, hasOthers = false, onLineComplete, silent = false, talking = false }: { speaker: NpcEncounter; line?: string | null; voiceId?: string | null; caption?: string | null; hasOthers?: boolean; onLineComplete?: () => void; silent?: boolean; talking?: boolean }) {
  ...
  useEffect(() => {
    ...
    lastSpokenKey = key
    let cancelled = false
    // She asks for the floor rather than seizing it. If Malachar or a player is
    // mid-line she waits; if she is cut off, the queue resumes her where she
    // stopped — which is what used to make NPCs seem voiceless.
    const handle = speak({
      rank: "npc",
