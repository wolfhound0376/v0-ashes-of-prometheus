...
const allegianceFor = (s) => s.allegiance ?? (s.kind === "character" ? "party" : "hostile")
const sideForRole = (r) => (String(r ?? "").trim().toLowerCase() === "ally/prisoner" ? "ally" : "hostile")
...
// ---- which side ----------------------------------------------------------
//
// THE BUG THESE EXIST FOR. The first cut defaulted the bestiary to hostile
// and NPCs to ally. Both were wrong against the live data: all 19
// npc_encounters rows have a NULL disposition, so Ilvara Mizzrym (CR 8) and
// the hook horror spawned as friends - and this bestiary is not a monster
// manual, since 18 of its 43 rows are "ally/prisoner" (the Velkynvelve cast),
// all of which spawned as enemies.

test("the Velkynvelve prisoners are allies", () => {
  // Stool, Jimjar, Buppido, Derendil, Eldeth, Ront... 18 of the 43 rows.
  assert.equal(sideForRole("ally/prisoner"), "ally")
})

test("enemies, bosses and named NPCs are hostile", () => {
  for (const r of ["enemy", "boss", "named_npc"]) {
    assert.equal(sideForRole(r), "hostile", `${r} is not hostile`)
  }
})

test("an unwritten role is hostile, not friendly", () => {
  // 17 of the 43 rows have a null role. The asymmetry is the point: an ally
  // spawned as an enemy is obvious on sight and one click to fix; a hostile
  // spawned as a friend sits in your initiative order, is skipped by every
  // targeting filter, and quietly makes the whole rehearsal meaningless.
  // When the data is missing, be wrong in the direction somebody notices.
  assert.equal(sideForRole(null), "hostile")
  assert.equal(sideForRole(undefined), "hostile")
  assert.equal(sideForRole(""), "hostile")
  assert.equal(sideForRole("some_role_nobody_has_written_yet"), "hostile")
})

test("the role is read however it was cased", () => {
  assert.equal(sideForRole("Ally/Prisoner"), "ally")
  assert.equal(sideForRole("  ALLY/PRISONER "), "ally")
})

test("the client and the server work the side out the same way", () => {
  // The dot on the button has to be the side you actually get. Two copies of
  // this rule that agree today is how they stop agreeing later, so both are
  // asserted to be the same one-line expression.
  const drawer = readFileSync(new URL("../../components/tactical/sandbox-drawer.tsx", import.meta.url), "utf8")
  const rule = /"ally\/prisoner"\s*\?\s*"ally"\s*:\s*"hostile"/
  assert.ok(rule.test(src), "the server no longer decides the side this way")
  assert.ok(rule.test(drawer), "the drawer no longer decides the side this way")
})

test("a bestiary row with no role is still hostile through allegianceFor", () => {
...
test("the three words are the board's own three words", () => {
  ...
  for (const r of ["ally/prisoner", "enemy", "boss", "named_npc", null, "nonsense"]) {
    assert.ok(words.has(sideForRole(r)), `role ${r} invented a side`)
  }
})

test("an unset NPC disposition falls through instead of answering", () => {
  // The exact bug: null meant "ally". An unset column is the ABSENCE of a
  // statement, not a statement that something is friendly - so it has to
  // return null and let the species role decide.
  const route = readFileSync(new URL("../../app/api/sandbox/route.ts", import.meta.url), "utf8")
  const m = /function dispositionSide[\s\S]*?\n\}/.exec(route)
  assert.ok(m, "dispositionSide is no longer findable")
  assert.ok(/if \(!s\) return null/.test(m[0]),
    "an unset disposition no longer falls through - check it does not mean ally again")
})
