...
const vsrc = _rf(new URL("../aoe-visual.ts", import.meta.url), "utf8")
...
const AREA_SPELLS = {
  ...
  "stinking cloud":   { school: "nature", concentration: true },
  "cloudkill":        { school: "nature", damage: "poison", concentration: true },
}
...
// The gas cloud had no caster until these two existed: no area spell in the
// book dealt poison or acid, so the miasma form was unreachable art.
test("the gas spells actually reach the cloud renderer", () => {
  for (const n of ["stinking cloud", "cloudkill"]) {
    assert.equal(kindFor(n, AREA_SPELLS[n]), "miasma", n)
    assert.equal(formFor(n, AREA_SPELLS[n]), "cloud", n)
    assert.equal(lingersFor(AREA_SPELLS[n]), true, `${n} must stay on the board`)
  }
  // Cloudkill routes by damage type; Stinking Cloud deals none, so it can
  // only be routed by name. If that entry is ever dropped it silently becomes
  // a blue "arcane" smudge, which is why this asserts them separately.
  assert.equal(BY_DAMAGE["poison"], "miasma")
  assert.equal(BY_SPELL["stinking cloud"], "miasma")
})
...
test("Fog Cloud is its own kind, not gloom", () => {
  // Sam: "cloud should definitely not look purple." Fog shared `gloom` with
  // darkness and necrotic; violet is right for a pall of decay and wrong for
  // weather.
  assert.match(vsrc, /"fog cloud":\s*"fog"/)
  assert.equal(/"fog cloud":\s*"gloom"/.test(vsrc), false)
})

test("fog's tint is a pale grey, with no violet in it", () => {
  const m = vsrc.match(/fog:\s*0x([0-9a-fA-F]{6})/)
  assert.ok(m, "fog needs a tint")
  const [r, g, b] = [0, 2, 4].map((i) => parseInt(m[1].slice(i, i + 2), 16))
  assert.ok(r > 190 && g > 190 && b > 190, `fog should be pale, got #${m[1]}`)
  // Violet is red and blue running ahead of green. Fog must not do that.
  assert.ok(g >= r, `fog must not be violet (r=${r} g=${g} b=${b})`)
})

test("fog hangs in the air like the gas does", () => {
  assert.match(vsrc, /fog:\s*"cloud"/)
})
